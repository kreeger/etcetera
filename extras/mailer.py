...
def error_mail(request):
	email_body = "There's been an error.\n\nPath:\t%s\nMethod:\t%s\nUser:\t%s\nHost:\t%s\n\nRequest dictionary:\n" % (request.build_absolute_uri(), request.method, request.user, request.get_host())
	...
	for user in User.objects.filter(groups__name="System Administrators"):
		emails.append(user.email)
	send_mail('[ETCETERA] Error at %s' % (request.build_absolute_uri(),), email_body, EMAIL_ADDRESS, emails, fail_silently=False)
