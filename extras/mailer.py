...
def wo_mail(work_order):
	email_body = "A new work order (order ID %i) created by %s %s \
		has been entered into Etcetera. Click here to view this ticket: \
		http://etc.missouristate.edu/etcetera/service/%i.\n\n \
		Thank you!\n \
		- Etcetera" % (
			work_order.id,
	...
	email_subject = "We've received your work order for the %s" % (
		work_order.equipment_text,
	)
	email_body = "%s,\nYour work order has been submitted successfully \
		and our technicians have been notified.\n\n \
		You will be periodically notified via email if there's a change in \
		status on your ticket. If at any other time you'd like to view your\
		ticket, click here: \
		http://etc.missouristate.edu/etcetera/service/%i.\n\n \
		Regards,\n \
